...
fn calculate_chunks(
    ...
    let chunk_positions: HashSet<_> = tileset.keys().copied().collect();
...
fn spawn_chunks(
    ...
    let chunk_package: Vec<(Position, Chunk, Image)> = to_spawn
        ...
        .map(|position| {
            ...
            let texture = chunk_to_image(&chunk, &config, *chunk_borders.get());
...
fn generate_chunk(
    ...
    let mut tiles = vec![vec![TileKind::Grass; config.chunk_tile_count]; config.chunk_tile_count];
...
fn chunk_to_image(chunk: &Chunk, config: &MapConfig, chunk_borders: ChunkBorderState) -> Image {
    ...
    // Short circuit and fill image completely with one colour if all tiles are the same, otherwise draw the tiles.
    ...
    if chunk_borders == ChunkBorderState::Shown {
        render_chunk_border(&mut dyn_image, config);
    }

    Image::from_dynamic(dyn_image, true, RenderAssetUsages::RENDER_WORLD)
}

fn render_chunk_border(chunk_image: &mut DynamicImage, config: &MapConfig) {
